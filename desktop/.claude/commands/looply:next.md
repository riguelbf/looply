---
description: Show the next recommended step for the current feature workflow
argument-hint: <feature-name> [session-label] [notes...]
---

Run the looply workflow `workflow-status` using the alias `/looply:next`.
...
- session-label: `$2` (optional)
- notes: `$3` (optional)

Usage:
- Host: `Claude Code`
- Alias: `/looply:next`
- Syntax: `/looply:next <feature-name> [session-label] [notes...]`

Example:
- `/looply:next pix-webhook-retry backend-afternoon`

When to use:
- Use when you only need the next recommended step, task and gate for a feature.

Expected output:
- The next recommended workflow, task and gate plus refreshed workflow state.

Suggested next step:
- Host: Claude Code. Execute the recommended next task or switch to `/looply:resume` for a fuller recap.
