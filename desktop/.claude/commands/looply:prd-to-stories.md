...
Run the looply workflow `prd-to-stories` using the alias `/looply:prd-to-stories`.
...
- prd-reference: `$2` (optional)
- notes: `$3` (optional)

Usage:
- Host: `Claude Code`
- Alias: `/looply:prd-to-stories`
- Syntax: `/looply:prd-to-stories <feature-name> [prd-reference] [notes...]`

Example:
- `/looply:prd-to-stories pix-webhook-retry prd-pix-webhook-retry`

When to use:
- Use when the PRD is approved and you need a delivery-ready story backlog.

Expected output:
- A story backlog and updated workflow state.

Suggested next step:
- Host: Claude Code. After story selection, run `/looply:story-to-production`.
