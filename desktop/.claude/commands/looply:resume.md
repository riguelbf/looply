---
description: Resume the current feature workflow from the persisted state
argument-hint: <feature-name> [session-label] [notes...]
---

Run the looply workflow `workflow-status` using the alias `/looply:resume`.
...
- Host: `Claude Code`
- Alias: `/looply:resume`
- Syntax: `/looply:resume <feature-name> [session-label] [notes...]`

Example:
- `/looply:resume pix-webhook-retry backend-afternoon`

When to use:
- Use when you want to continue a feature from the persisted state of the current session or feature.

Expected output:
- A resumed workflow summary plus refreshed workflow state.

Suggested next step:
- Host: Claude Code. Continue with the task and workflow returned by the resumed state.
