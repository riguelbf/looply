...
Run the looply workflow `workflow-status` using the alias `/looply:workflow-status`.
...
Usage:
- Host: `Claude Code`
- Alias: `/looply:workflow-status`
- Syntax: `/looply:workflow-status <feature-name> [session-label] [notes...]`

Example:
- `/looply:workflow-status pix-webhook-retry`

When to use:
- Use when you need to resume work or inspect the current state of a feature.

Expected output:
- A short decision summary and refreshed workflow state.

Suggested next step:
- Host: Claude Code. Continue with the workflow recommended in the state file using the host-specific alias.
