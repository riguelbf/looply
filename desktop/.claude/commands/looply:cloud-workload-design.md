...
Run the looply workflow `cloud-workload-design` using the alias `/looply:cloud-workload-design`.
...
- scope-reference: `$2` (required)
- constraints: `$3` (optional)

Usage:
- Host: `Claude Code`
- Alias: `/looply:cloud-workload-design`
- Syntax: `/looply:cloud-workload-design <feature-name> <scope-reference> [constraints...]`
...
When to use:
- Use when the main open question is cloud topology, async-first communication, governance controls or cost posture for a workload.

Expected output:
- A cloud tech spec, a cloud ADR and governance or cost review artifacts.

Suggested next step:
- Host: Claude Code. After cloud planning, continue with `/looply:story-to-production` or revisit `/looply:workflow-status` for the persisted next step.
