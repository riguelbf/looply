...
function renderContextIndexMarkdown(input: ContextIndexInput): string {
  ...
  return [
    ...
    "2. Context ledger: `.looply/custom/features/<feature-name>/context-ledger.db` (accumulated decisions across all stages, SQLite — use `looply ledger read --summary-only`)",
    ...
    "- `.looply/custom/features/<feature-name>/context-ledger.db` (SQLite append-only shared memory — use `looply ledger read`)",
