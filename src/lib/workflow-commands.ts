...
export function renderClaudeWorkflowCommand(input: {
  ...
  const lines = [
    ...
    `- Context ledger: \`.looply/custom/features/<feature-name>/context-ledger.db\` (SQLite, use \`looply ledger\` commands)`,
    "- Read via `looply ledger read --feature $1 [--summary-only]`. Append via `looply ledger append --feature $1 --stage <name> --decision <text> --rationale <text> --constraints <text> --risks <text>`.",
  ...
  lines.push(
    ...
    "3. Create or update the feature state file before deciding the next step. Also read and maintain the context ledger via `looply ledger read --feature <feature-name>` and `looply ledger append --feature <feature-name> ...`.",
    ...
    "8. Update the feature state file after every relevant transition. Append your stage decisions to the context ledger via `looply ledger append ...`. Update the summary via `looply ledger summary update ...`.",
...
export function renderCodexWorkflowCommand(input: {
  ...
  const lines = [
    ...
    "Context ledger: .looply/custom/features/<feature-name>/context-ledger.db (SQLite, use looply ledger commands)",
    "Read via `looply ledger read --feature <feature-name> [--summary-only]`. Append via `looply ledger append --feature <feature-name> --stage <name> --decision <text> --rationale <text> --constraints <text> --risks <text>`.",
    "Session links file: .looply/custom/session-links.json",
    "",
    "Argument mapping:"
  ];
  ...
  lines.push(
    ...
    "7. Update the feature state file after every relevant transition. Append your stage decisions to the context ledger via `looply ledger append ...`. Update the summary via `looply ledger summary update ...`.",
...
export function renderCodexSkillDocument(input: {
  ...
  const lines = [
    ...
    `- Managed pack: ${input.packReference}`,
    `- Workflow state template: ${input.stateTemplateReference}`,
    `- Context ledger: .looply/custom/features/<feature-name>/context-ledger.db (SQLite, use looply ledger read/append/summary)`,
    `- Custom overrides: ${input.customReference}`,
    `- Execution hints: ${input.hintsReference}`,
    `- Example hints: ${input.exampleHintsReference}`,
    ...
    "",
    "Execution rules:",
    "1. Start by reading the workflow playbook, the host status contract if it exists, the feature state file if it already exists, and the context ledger via `looply ledger read --feature <feature-name> [--summary-only]`.",
    ...
    "4. After completing a workflow stage, append decisions, rationale, constraints and risks via `looply ledger append --feature <feature-name> --stage <stage> --decision <text> --rationale <text> --constraints <text> --risks <text>`. Update the summary via `looply ledger summary update --feature <feature-name> --text <summary>`.",
