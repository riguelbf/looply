...
export class FileHostPublisher implements HostPublisher {
  ...
  private renderEntrypoint(input: {
    ...
    return [
      ...
      ...(input.host === "opencode"
        ...
        : []),
      "",
      "Knowledge graph and code context policy:",
      ...
      "18. When you need symbol-level detail, entrypoints or module relations, consult `.looply/state/code-context.json` as a structured accelerator.",
      "",
      "Context ledger policy:",
      "19. Every feature has a shared context ledger at `.looply/custom/features/<feature-name>/context-ledger.db` (SQLite).",
      "20. Always read the summary before starting work: `looply ledger read --feature <feature-name> --summary-only`.",
      "21. After completing a workflow stage, append via `looply ledger append --feature <feature-name> --stage <name> --decision <text> --rationale <text> --constraints <text> --risks <text>`. Update the summary via `looply ledger summary update --feature <feature-name> --text <text>`.",
      "22. On low context_budget, use `--summary-only`. On medium or high budget, read the full ledger (omit `--summary-only`).",
      "",
      "Pre-Action Gate (enforced before ANY code change or artifact creation):",
      "1. Read `.looply/state/context-index.md` to discover active features and context priority.",
      "2. Read `.looply/custom/session-links.json` and bind this session to the correct feature.",
      "3. If a feature is bound, read its `workflow-status.md` AND run `looply ledger read --feature <feature-name> --summary-only` before acting.",
      "4. If no feature is bound and the user requests a code change, suggest `$looply-idea-to-prd` or `$looply-workflow-status` to establish structured context first.",
      "5. For existing projects, treat the real local codebase as the primary source of truth. Context files are accelerators — never override the codebase when stale or inconsistent.",
      "6. On medium+ context_budget, also read `knowledge-graph.json` and `code-context.json` for structural awareness.",
      "",
  ...
  private async writeWorkflowCommands(input: {
    ...
    if (input.host === "codex" || input.host === "opencode") {
      ...
      const skillSearchFiles = await this.writeSkillSearchSkill({
        targetRoot: input.targetRoot,
        scope: path.basename(input.targetRoot) === ".codex" || path.basename(input.targetRoot) === ".opencode" ? "global" : "project",
        outputLocale: input.outputLocale,
        projectMode: input.projectMode,
      ...
      additionalFiles.push(...skillSearchFiles);
    }
...
`;
  ...
  private async writeSkillSearchSkill(input: {
    targetRoot: string;
    scope: "project" | "global";
    outputLocale: "en" | "pt-BR";
    projectMode: "existing-project" | "greenfield";
    ...
    const skillsRoot = this.resolveCodexSkillsRoot(input.targetRoot, input.scope);
    const skillRoot = path.join(skillsRoot, "looply-skill-search");
    const agentsRoot = path.join(skillRoot, "agents");
    const skillFile = path.join(skillRoot, "SKILL.md");
    const metadataFile = path.join(agentsRoot, "openai.yaml");
    const hostContractFile = path.join(input.targetRoot, "HOST_CONTRACT.md");
    const commandsIndexFile = path.join(input.targetRoot, "LOOPLY_COMMANDS.md");
    ...
    const skillContent = [
      ...
      "",
      "Primary references:",
      `- Skills directory: ${relativePathForDisplay(skillRoot, skillsRoot)}`,
      `- Command index: ${relativePathForDisplay(skillRoot, commandsIndexFile)}`,
      `- Workflow playbook: ${relativePathForDisplay(skillRoot, input.workflowPlaybookFile)}`,
      `- Host status contract: ${relativePathForDisplay(skillRoot, path.join(input.targetRoot, ".looply", "state", "host-status-contract.json"))}`,
      `- Host contract: ${relativePathForDisplay(skillRoot, hostContractFile)}`,
      "- Entrypoint: ../../../AGENTS.md",
      `- Context index: ${relativePathForDisplay(skillRoot, path.join(input.targetRoot, ".looply", "state", "context-index.md"))}`,
      `- Project context: ${relativePathForDisplay(skillRoot, path.join(input.targetRoot, ".looply", "custom", "project-context.md"))}`,
      `- Session context: ${relativePathForDisplay(skillRoot, path.join(input.targetRoot, ".looply", "custom", "session-context.md"))}`,
      `- Interaction policy: ${relativePathForDisplay(skillRoot, path.join(input.targetRoot, ".looply", "state", "interaction-policy.json"))}`,
      "",
      "## Purpose",
      "",
      ...
      input.projectMode === "existing-project"
        ...
        : "6. For greenfield projects, use managed artifacts and explicit assumptions until a codebase exists.",
      "7. If context files are empty, draft, or stale, validate against the codebase before trusting them.",
      `8. Follow ${input.interactionMode} interaction mode to avoid unnecessary repeated clarifications.`,
    ...
    const metadataContent = [
      "interface:",
      "  display_name: \"$looply-skill-search\"",
      "  short_description: \"Discover available looply skills and workflows. Forces skill-aware discovery before any code action.\"",
      "  brand_color: \"#7C3AED\"",
      "  default_prompt: \"$looply-skill-search\"",
      "policy:",
      "  allow_implicit_invocation: true"
    ].join("\n");

    await fs.writeFile(skillFile, skillContent, "utf8");
    await fs.writeFile(metadataFile, metadataContent, "utf8");
